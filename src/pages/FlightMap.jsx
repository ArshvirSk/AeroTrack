...
const getPlaneIcon = (angle) => {
  // Create a custom SVG icon with rotation applied directly in the SVG
  const svgString = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="#FFD700" 
         style="filter: drop-shadow(0 0 2px rgba(255, 215, 0, 0.7)); transform: rotate(${angle}deg); transform-origin: center;">
...
const generateCurve = (start, end, segments = 20) => {
  const toRadians = (deg) => (deg * Math.PI) / 180;
  const toDegrees = (rad) => (rad * 180) / Math.PI;
  ...
  for (let i = 0; i <= segments; i++) {
    const f = i / segments;
    const A = Math.sin(((1 - f) * Math.PI) / 2);
...
// Mock function to return coordinates based on IATA codes
  ...
  const generateRoute = () => {
    ...
    setTimeout(() => {
      setRoute({
        from: originAirport,
        to: destAirport,
        path: [originAirport.position, destAirport.position],
        distance: Math.floor(
