...
const Flights = () => {
  ...
  useEffect(() => {
    ...
    // Load initial flights from backend API
    fetchFlights()
      .then((flights) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      // Get all flights from backend and filter client-side
      const allFlights = await fetchFlights();
      
      // Filter flights based on search criteria if origin/destination are specified
      let filteredFlights = allFlights;
      
      if (origin || destination) {
        filteredFlights = allFlights.filter((flight) => {
          const departureMatch = !origin || 
            flight.departure?.iata?.toLowerCase().includes(origin.toLowerCase()) ||
            flight.departure?.airport?.toLowerCase().includes(origin.toLowerCase());
          
          const arrivalMatch = !destination ||
            flight.arrival?.iata?.toLowerCase().includes(destination.toLowerCase()) ||
  ...
  const handleBooking = (flight) => {
    const searchQuery = `${flight.airline?.name || "Unknown Airline"} ${
      flight.flight?.iata || ""
    } from ${flight.departure?.iata || "Unknown"} to ${
